...
if(isset($_POST['create-submit'])){
    ...
    if($result){
        ?>
        <script>location.replace("task_list.php");</script>
        <?php
    }
}
        ...
        <div class="row">
            <div class="col">
                <div class="today-tasks">
                    <h3 class="title">Today <span><?php echo($count) ?></span></h3>
...
?>
                        <!-- tcard<?php echo($row['task_id']) ?>-->
                        <div id="tcard<?php echo($row['task_id']) ?>" class="tcard">
                            <div class="ttitle"><span><?php echo(ucwords($row['title'])) ?></span><i class="showdialog fa-solid fa-ellipsis-vertical"></i></div>
                            <div class="ttime"><span><?php echo($start . ' - ' . $end) ?></span><span><?php echo($date->format('F d,Y')) ?></span></div>
                            <div class="tdescription"><span><?php echo($row['description']) ?></span></div>
                            <div class="dialogpop">
                                <button type="button" onclick="add_to_calender(this,<?php echo($row['task_id']) ?>)" class="ep-top">Add to calender</button>
                                <div class="ep-bottom">
                                    <span onclick="delete_task('tcard<?php echo($row['task_id']) ?>',<?php echo($row['task_id']) ?>)" class="ep-delete">Delete</span>
                                    <span onclick="edit_task(<?php echo($row['task_id']) ?>)" class="ep-edit">Edit</span>
                                </div>
                            </div>
                        </div>
                        <!-- tcard <?php echo($row['task_id']) ?>-->
        ...
        <div class="row">
            <div class="col">
                <div class="upcoming-tasks">
                    <h3 class="title">Upcoming <span><?php echo($count) ?></span></h3>
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            ?>
                        ...
                        <div id="tcard<?php echo($row['task_id']) ?>" class="tcard">
                            ...
                            <div class="tdescription"><span><?php echo($row['description']) ?></span></div>
                            <div class="dialogpop">
                                <button class="ep-top">Add to calender</button>
                                <div class="ep-bottom">
                                    <span onclick="delete_task('tcard<?php echo($row['task_id']) ?>',<?php echo($row['task_id']) ?>)" class="ep-delete">Delete</span>
                                    <span onclick="edit_task(<?php echo($row['task_id']) ?>)" class="ep-edit">Edit</span>
                                </div>
...
<?php } ?>
            ...
            <div class="col">
                <div class="create-task">
                    <div id="create-task" class="create">
                        ...
                        <img src="./images/create-task.svg" alt="">
                    </div>
                </div>
            </div>
        </div>
                ...
                <div class="col-6">
                    <div class="input-group">
                        <div class="input">
                            ...
                            <input type="text" name="task-title" id="task-title" placeholder="Type the title here..." required>
                        </div>
                        <div class="input">
                            <label for="task-description">Description</label>
                            <textarea name="task-description" id="task-description" cols="30" rows="5" placeholder="Type the description here..." required></textarea>
                        </div>
                    </div>
                </div>
            ...
            <div class="row">
                <div class="col-6">
                    <div class="input-group">
                        <div class="input">
                            <label for="task-title">Title</labtiel>
                            <input type="text" name="edit-task-title" id="edit-task-title" placeholder="Type the title here..." required>
                        </div>
                        <div class="input">
                            <label for="task-description">Description</label>
                            <textarea name="edit-task-description" id="edit-task-description" cols="30" rows="5" placeholder="Type the description here..." required></textarea>
                        </div>
                    </div>
                </div>
                <div class="offset-1 col-5">
    ...
    <script>
        ...
        function delete_task(t,t_id){
            ...
            type: "POST",
            url: "task_list.php",
            data: {delete_task: true,t_id: t_id , dietitianuserID:'<?php echo($dietition) ?>'},
        ...
        function edit_task(t_id){
            ...
            type: "POST",
            url: "task_list.php",
            data: {edit_task: true,t_id: t_id , dietitianuserID:'<?php echo($dietition) ?>'},
            success: function(data) {
                // Handle the response here
        ...
        function add_to_calender(dialog,task_id){
            ...
            type: "POST",
            url: "task_list.php",
            data: {add_calender: true,task_id: task_id },
            success: function(data) {
                // Handle the response here
                console.log(data);
                console.log(task_id);
